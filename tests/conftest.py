"""
Shared fixtures for the presto test suite.

Holds the test problems (with analytic derivatives, so nothing falls back to
finite differences), matrix generators covering the three cases a trust-region
subproblem solver has to handle, and an independent reference solver for that
subproblem so the fast solvers can be checked against the exact answer rather
...
def spd(n, cond=50.0, seed=SEED):
    """Symmetric positive definite with a prescribed 2-norm condition number."""
...
def indefinite(n, seed=SEED):
    """Symmetric with mixed-sign eigenvalues: lambda_min < 0 < lambda_max."""
    Q = np.linalg.qr(np.random.default_rng(seed).standard_normal((n, n)))[0]
...
def hard_case(n=3, seed=SEED):
    ...
    coeffs = np.ones(n)
    coeffs[0] = 0.0                                   # kill the lambda_min direction
    g = Q @ coeffs
...
def subproblem_reference(g, B, radius, tol=1e-13, max_iter=300):
    """
    Exact solution of  min gp + 1/2 pBp  s.t.  ||p|| <= radius, from the
    eigen-expansion plus bisection on lambda. Independent of everything under
    test: no Cholesky, no Newton on the secular equation, no QR.
    """
    d, V = np.linalg.eigh(B)
    q = V.T @ np.asarray(g, dtype=float)

    if d[0] > 0:                                       # interior candidate
        p = -V @ (q / d)
    ...
    if norm < radius * (1.0 - 1e-8):                   # hard case correction
...
def _quadratic(n=4, cond=50.0):
    ...
    # as a second positional argument (see test_hessian_call_contract).
    ...
    return SimpleNamespace(func=f, grad=f.gradient, hess=f.hessian, A=A, b=b,
                           n=n, x0=np.zeros(n), x_star=x_star, f_star=f(x_star))
...
def _rosenbrock():
    ...
    return SimpleNamespace(func=rosenbrock, grad=rosenbrock.gradient,
                           ...
                           x_star=np.ones(2), f_star=0.0)


def _rosenbrock_residual():
    """
    Residual form with 0.5||r||^2 == rosenbrock(x, a, b) exactly, so a least
    squares run is comparable against the scalar drivers on the same problem.
    """
    a, b = 1.0, 100.0
    ...
    return SimpleNamespace(func=r, jac=jac, n=2, m=2, x_star=np.ones(2),
                           x0=np.array([1.2, 1.2]), x0_hard=np.array([-1.2, 1.0]))
