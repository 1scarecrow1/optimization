...
eigen-expansion + bisection); the approximate ones are checked against two properties -
stay inside the region, and achieve at least the Cauchy-point decrease.
"""
import numpy as np
import pytest
...
def test_generalised_cauchy_respects_elliptical_region(rad):
    """With scaling D the constraint is ||Dp|| <= radius"""
    n = 4
