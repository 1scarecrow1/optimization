...
def plot_cg_convergence(A, b, result, x_opt=None, spectrum='A',
                        spectrum_style='stem', log=np.log, ax=None, title=None):
    """
    Plot log of the squared energy-norm error  ||x_k - x*||_A^2 = e^T A e
    (objective function of CG) vs iteration, and overlay the eigenvalue
    spectrum that controls the convergence rate.

    spectrum :       'A'              - eigenvalues of original matrix
                     'preconditioned' - eigenvalues of preconditioned matric
    spectrum_style : 'stem'           - one vertical line per eigenvalue 
...
def plot_nonlinear_cg_convergence(result, f_min=None, log=np.log,
    ...
    convergence metrics vs iteration:
        (a) log ||grad f(x_k)||
        (b) log (f(x_k) - f*)   (optimality gap; f* defaults to min f seen)
    """
    iters = result.iterations
    ks = np.array([it['iter'] for it in iters])
    grads = stack(iters, 'grad')
