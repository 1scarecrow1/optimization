...
def plot_fit(t, y, model, theta, theta_true=None, iterations=None, n_trace=4,
             title=None, ax=None):
    """Data, fitted curve, optional true curve, and a faded trace of early iterates."""
    if ax is None:
...
def plot_residuals(t, r, title=None, axes=None):
    """Residuals vs t (structure => model misspecification) and their distribution."""
...
def plot_lstsq_comparison(A, b, solutions, reference='lstsq', axes=None):
    """‖Ax-b‖ per solver, and per-solver deviation from the reference solution."""
